...
void reactor::task_queue::do_dump() const {
    ...
    if (!seastar_logger.log(log_level::warn, rate_limit, "Too long queue accumulated for {} ({} tasks)", _name, _q.size())) {
        // The first log message was not logged (rate limit was hit, an exception occured or warn level is disabled), so we skip rest as well.
        return;
    }
