...
static inline bool is_internal(const sstring& name) {
    if (name.c_str()[0] == '_') [[unlikely]] {
        return boost::algorithm::starts_with(name, "__");
    }
    return false;
}
...
class metrics_handler : public httpd::handler_base  {
    sstring _prefix;
    config _ctx;
    static const std::function<bool(const mi::labels_type&)> _true_function;
...
const std::function<bool(const mi::labels_type&)> metrics_handler::_true_function = [](const mi::labels_type&) {
    return true;
};
