...
// Owned by no smart pointer: kept alive via intrusive links (_fq_entry,
// _intent) until dispatch(). cancel() must not delete `this`, since the
// caller still holds a reference to it; it only sets `_cancelled`, and
// dispatch() does the actual delete (directly if cancelled, else via the
// sink's complete()/set_exception()).
