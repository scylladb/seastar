        ...
        for (unsigned si = 0; si < _tokens.size(); si++) {
            ...
            while (available > 0) {
                ...
                for (auto* child : _children) {
                    ...
                    if (ov > 0) {
                        overflowed_shares += child->shares();
                        overflow += ov;
                    }
                    distributed += amount;
                }
                ...
                taker_shares -= overflowed_shares;
                available = overflow;
            }
        }
        for (auto* child : _children) {
...
public:
    ...
    unsigned pc_id() const noexcept { return _pc.id(); }
...
queued_io_request::queued_io_request(internal::io_request req, io_queue& q, io_queue::capacity_t cap, io_queue::priority_class_data& pc, io_direction_and_length dnl, iovec_keeper iovs)
...
{
    SEASTAR_IO_TRACE(io_queue_queued, fd(), _desc.get(), dnl.rw_idx(), pc.pc_id(), offset(), dnl.length());
}
