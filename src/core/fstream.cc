...
data_source make_file_data_source(file f, uint64_t offset, uint64_t len, file_input_stream_options opt) noexcept {
    return data_source(std::make_unique<file_data_source_impl>(std::move(f), offset, len, std::move(opt)));
}

data_source make_file_data_source(file f, file_input_stream_options opt) noexcept {
