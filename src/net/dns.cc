    ...
    , _timer(std::bind(&impl::poll_sockets, this))
{
    ...
    static const ares_socket_functions_ex callbacks_ex = {
        ...
        .asetsockopt = [](ares_socket_t s, ares_socket_opt_t opt, const void * val, ares_socklen_t val_size, void * p) {
            // Most socket options are handled at the Seastar socket layer
            // Log what c-ares is trying to set for debugging purposes
            dns_log.trace("c-ares socket option request: fd={} opt={} size={}", s, static_cast<int>(opt), val_size);

            // We could potentially handle some options here if needed:
            // - ARES_SOCKET_OPT_SENDBUF_SIZE / ARES_SOCKET_OPT_RECVBUF_SIZE
            // - ARES_SOCKET_OPT_BIND_DEVICE
            // For now, return ENOSYS to indicate not supported
            // c-ares will handle this gracefully according to the documentation
            errno = ENOSYS;
            return -1;
        },
