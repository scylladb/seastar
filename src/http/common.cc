...
public:
    ...
    virtual future<> put(net::packet data) override {
        auto size = data.len();
        if (size == 0 || _bytes_written == _limit) {
            return make_ready_future<>();
        }
        if (_bytes_written + size > _limit) {
            return make_exception_future<>(std::runtime_error(format("body conent length overflow: want {} limit {}", _bytes_written + size, _limit)));
        }
