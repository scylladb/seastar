...
The project uses CMake with ninja as the build system. Build directories already exist in `/build/<mode>`.
...
- **Check for existing build directories first** - the user will often have already configured the project. If a `build/` directory exists, use that instead of running `./configure.py`
- **Always use `ninja -C build/<mode>`** - the build directory already exists
- **Do NOT use** plain `ninja` from the root directory - it will fail with "Execute Ninja in a build directory"
- The working directory during builds should typically be the project root (`/home/tdowns/dev/seastar`)
...
Comment all public methods carefully following the style the same source file (e.g., doxygen if existing comments use that style).
