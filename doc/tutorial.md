...
`coroutine::at_coroutine_exit` is a facility to make this easier.
...
The lambda passed to `coroutine::at_coroutine_exit` will be run when the coroutine frame is still live, so capturing variables by references is allowed.
