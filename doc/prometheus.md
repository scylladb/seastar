...
`local_shard_only::yes` when defining it:
...
from every shard as before. The hint must be set on every registration of
the family, on every shard that registers it: a family hinted on one
shard but registered without the hint on another will have those other
series silently omitted from an exact-name scrape.
