...
The `asymmetric_io_uring` reactor backend in Seastar is designed to optimize application performance by offloading kernel-side networking and disk I/O processing from compute-bound application shards onto a dedicated set of CPU cores, referred to as **worker cores**.
...
The `io_uring` asynchronous worker threads and polling threads are pinned to a user-defined set of CPUs (`--async-workers-cpuset`). Typically, these cores are aligned with the system's **networking cores** (cores configured to handle network interface card IRQs using the `perftune.py` script) and are ideally excluded from the main Seastar application cpuset. However, exclusion is not strictly enforced; if the worker cpuset overlaps with the application shard cpuset, Seastar will log a configuration warning due to the risks of CPU contention.
...
1. **Topology Detection**: The backend uses the Portable Hardware Locality (`hwloc`) library to detect NUMA nodes, cores, and SMT sibling threads. If `hwloc` is not available, the backend falls back to a topology-agnostic greedy assignment (effectively treating the system as a single NUMA node and disabling SMT sibling evaluation).
...
* *Note: `taskset` refers to the external Linux OS utility/affinity mask constraint applied to the process, rather than a native Seastar command-line option.*