...
future<R> finally_coroutine(canary c, std::exception_ptr ex, Func&& func) {
    coroutine::finally(std::move(func));
...
struct finally_tester {
    future<> do_run(bool throw_exception, std::source_location sl) {
        ...
        std::exception_ptr ex;
        if (throw_exception) {
            ex = std::make_exception_ptr(std::runtime_error("finally test"));
        }

        bool failed = false;
        if constexpr (FuncTakesException) {
            std::exception_ptr ex_out;
            auto f = co_await coroutine::as_future(finally_coroutine<R>(canary{destroyed}, ex, [&destroyed, &called, &ex_out] (std::exception_ptr ex) mutable -> future<> {
                ex_out = std::move(ex);
                return yield().then([&] {
                    called = true;
                });
            ...
            if (throw_exception) {
                BOOST_REQUIRE(bool(ex_out));
                BOOST_REQUIRE_EQUAL(fmt::to_string(ex), fmt::to_string(ex_out));
            }
        } else {
            auto f = co_await coroutine::as_future(finally_coroutine<R>(canary{destroyed}, ex, [&destroyed, &called] () mutable -> future<> {
                return yield().then([&] {
                    called = true;
                });
            }));
            failed = f.failed();
            if (failed) {
                fmt::print("finally_tester caught exception: {}\n", f.get_exception());
            } else {
                f.ignore_ready_future();
            }
        }
        BOOST_REQUIRE_EQUAL(failed, throw_exception);
