...
SEASTAR_THREAD_TEST_CASE(gate_holder_closed_test) {
    gate g;
    g.close().get();
    BOOST_REQUIRE_THROW(g.get_holder(), gate_closed_exception);
}

SEASTAR_THREAD_TEST_CASE(gate_holder_move_test) {
    gate g;
    auto gh0 = g.get_holder();
    auto fut = g.close();
    BOOST_CHECK(!fut.available());
...
SEASTAR_THREAD_TEST_CASE(gate_holder_copy_and_move_test) {
    gate g0;
    auto gh00 = g0.get_holder();
    auto gh01 = gh00;
    auto fut0 = g0.close();
    BOOST_CHECK(!fut0.available());
    gate g1;
    auto gh1 = g1.get_holder();
    auto fut1 = g1.close();
...
SEASTAR_THREAD_TEST_CASE(gate_holder_copy_after_close_test) {
    ...
    auto fut = g.close();
    gate::holder gh1;
    BOOST_REQUIRE_THROW(gh1 = gh0, gate_closed_exception);
    BOOST_CHECK(!fut.available());
    gh0.release();
    fut.get();
}
