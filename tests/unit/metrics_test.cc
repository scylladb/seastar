...
SEASTAR_THREAD_TEST_CASE(test_renaming_execution_stage) {
    using namespace seastar;
    sstring sg_name1 = "sg1";
    sstring sg_name2 = "sg2";
    inheriting_concrete_execution_stage<void> stage{"my_stage", []{}};
    scheduling_group sg = create_scheduling_group(sg_name1, 111).get();
    ...
    BOOST_REQUIRE(label_vals.find(sstring("my_stage.") + sstring(sg_name1)) != label_vals.end());

    rename_scheduling_group(sg, sg_name2).get();

    label_vals = get_label_values(sstring("execution_stages_tasks_scheduled"), sstring("execution_stage"));
    BOOST_REQUIRE(label_vals.find(sstring("my_stage.") + sstring(sg_name1)) == label_vals.end());
    BOOST_REQUIRE(label_vals.find(sstring("my_stage.") + sstring(sg_name2)) != label_vals.end());
}
