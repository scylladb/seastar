...
static future<> do_test_fairness() {
...
SEASTAR_TEST_CASE(test_wakeups) {
    ...
    std::default_random_engine random_engine(testing::local_random_engine());
    std::uniform_int_distribution<unsigned> ng_dist(0, groups.size() - 1);
    ...
    for (unsigned test = 0; test < 32; test++) {
        ...
        for (unsigned i = 0; i < waiters.capacity(); i++) {
            ...
            for (unsigned w = 0; w < nw; w++) {
                ...
                (void)with_scheduling_group(groups[g], [&s = ws.sem, &ready, &done, i, w, g] () mutable {
                    ready.signal();
        ...
        for (unsigned i = 0; i < waiters.size(); i++) {
            auto g = ng_dist(random_engine);
            (void)with_scheduling_group(groups[g], [&s = waiters[i], &done] () mutable {
                s.sem.signal(s.nr);
                done.signal();
            });
        }
