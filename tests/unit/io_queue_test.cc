...
SEASTAR_THREAD_TEST_CASE(test_disk_level_rate_limiting) {
    ...
    // stay below max_request_length so that no splitting occurs.
    {
        ...
        auto bw = run_and_check_bandwidth(tio, get_default_pc(), bw_limit * 0.9, 4, std::chrono::seconds(5), req_size).get();
    ...
    // run_and_check_bandwidth measures bytes/s; dividing by req_size gives IOPS.
    {
        ...
        auto iops_as_bw = run_and_check_bandwidth(tio, get_default_pc(), iops_limit * req_size * 0.9, 8, std::chrono::seconds(5), req_size).get();
...
SEASTAR_THREAD_TEST_CASE(test_idle_class_does_not_waste_tokens) {
    ...
    auto f_b = run_and_check_bandwidth(tio, pc_b, bw_limit / 2, 8, std::chrono::seconds(5), req_size);
