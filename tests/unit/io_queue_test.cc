...
SEASTAR_THREAD_TEST_CASE(test_destroy_priority_class_with_requests) {
    ...
    // Push this request to executing
    tio.queue.poll_io_queue();

    auto fq = tio.queue_request(pc,
        internal::io_direction_and_length(internal::io_direction_and_length::read_idx, 0), 
        internal::io_request::make_write(0, 0, nullptr, 1, false),
        nullptr, {});

    // At this point the class has both -- executing and queued requests
    // This io-queue is not registered in reactor, so kick it here
    ...
    try {
        fq.get();
    } catch (seastar::cancelled_error&) {
        was_canceled = true;
    }
    BOOST_REQUIRE(was_canceled);
