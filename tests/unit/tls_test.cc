...
SEASTAR_THREAD_TEST_CASE(test_reload_certificates_with_creds) {
    ...
    auto certs = b.build_reloadable_server_credentials([&](const std::unordered_set<sstring> &files,
        ...
        using opt_info_v = std::optional<std::vector<seastar::cert_info>>;

        auto certs_info = [&creds] () -> opt_info_v {
            try {
                return creds.get_cert_info();
            } catch (...) {
                return std::nullopt;
            }
        }();
        ...
        auto trust_list_info = [&creds] () -> opt_info_v {
            try {
                return creds.get_trust_list_info();
            } catch (...) {
                return std::nullopt;
            }
        }();
