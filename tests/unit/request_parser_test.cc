...
SEASTAR_TEST_CASE(test_request_size_limit_scattered) {
    ...
    BOOST_REQUIRE(std::holds_alternative<continue_consuming>(feed(parser, part1).get()));
    BOOST_REQUIRE(!parser.size_limit_exceeded());
    BOOST_REQUIRE(std::holds_alternative<stop_consuming<char>>(feed(parser, part2).get()));
    BOOST_REQUIRE(parser.size_limit_exceeded());
