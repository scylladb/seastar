...
struct metrics_perf_fixture {
    ...
    seastar::future<size_t> run_metrics_bench(
        size_t group_count, size_t families_per_group, size_t series_per_family, data_type type, bool enable_aggregation = false) {
        ...
        for ([[maybe_unused]] auto _: irange(iterations)) {
            ...
            co_await access{}.write_body(config, false, "", false, true,
