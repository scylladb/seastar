...
private:
    ...
    std::unique_ptr<retry_strategy<std::exception_ptr>> _connection_reset_strategy;
...
public:
    ...
    explicit client(std::unique_ptr<connection_factory> f, unsigned max_connections, size_t max_bytes_to_drain, std::unique_ptr<retry_strategy<std::exception_ptr>>&& retry_strategy);
