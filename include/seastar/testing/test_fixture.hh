...
namespace seastar::testing {
    namespace detail {
        // could/should make futurize::invoke use std::invoke so
        // it can handle member function, but don't want to change
        // all code in the universe for this here.
        template<typename Func, typename... Args>
        future<> invoke(Func&& f, Args&&... args) {
            using ret_type = decltype(std::invoke(f, std::forward<Args>(args)...));
        ...
        future<> do_fixture_test(Func&& f, T& t, const sstring& name) {
            return conditional_invoke_setup(t).then([&] {
                return invoke(f, t);
            }).finally([&]() {
    ...
    public:
        ...
        async_class_based_fixture(const Args& ...args)
    ...
    inline boost::unit_test::decorator::fixture_t async_fixture(const Args& ...args) {
...
#define SEASTAR_FIXTURE_TEST_CASE(name, F, ...)                     \
    ...
    SEASTAR_TEST_CASE(name, ##__VA_ARGS__) {                        \
        ...
        return td::do_fixture_test(&type::run_test_case, *t, #F)    \
...
#define SEASTAR_FIXTURE_THREAD_TEST_CASE(name, F, ...)              \
    ...
    SEASTAR_TEST_CASE(name, ##__VA_ARGS__) {                        \
        return seastar::async([] {                                  \
            ...
            using type = name ## _fxt;                              \
            type t;                                                 \
            td::do_fixture_test(&type::run_test_case, t, #F).get(); \
        });                                                         \
    }                                                               \
