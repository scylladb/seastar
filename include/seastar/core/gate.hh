...
public:
    ...
    public:
        ...
        /// May throw \ref gate_closed_exception if the gate is already closed
        /// and the copied \ref gate is different than the current one.
        holder& operator=(const holder& x) {
            if (x._g != _g) {
                if (x._g) {
                    x._g->enter();
                }
    ...
    /// Enter the gate and get a gate::holder object
    /// that will leave the gate when destroyed
    holder get_holder() {
        return holder(*this);
    }
};
