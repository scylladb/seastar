...
/*!
 * \brief Used to hold internalized_labels_ref in a std::map
 *
 * A wrapper around `internalized_labels_ref` that enables transparent lookup
 * against `labels_type` such that we can do no-alloc lookups where
 * `internalized_labels` would be used as a key in a std::map
 */
