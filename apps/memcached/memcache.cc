...
using matrix_of_buffers = std::vector<std::vector<seastar::temporary_buffer<char>>>;
namespace seastar::util { void append_buffers(matrix_of_buffers& vb, std::span<seastar::temporary_buffer<char>> bufs); }
...
private:
    ...
    struct connection {
        ...
        connection(ipv4_addr src, uint16_t request_id, input_stream<char>&& in, size_t out_size,
            ...
            , _in(std::move(in))
            , _out(output_stream<char>(data_sink(std::make_unique<util::basic_memory_data_sink<matrix_of_buffers>>(_out_bufs)), out_size, make_opts()))
...
namespace seastar::util {
void append_buffers(matrix_of_buffers& vb, std::span<temporary_buffer<char>> bufs) {
